...
let output = function (data){
  ...
  for(let i = 0;i < data.results.length;i++){
    if(data.results[i].gender === 'female'){
      let personInfo = data.results[i]

      let {gender, name, location, email, login, dob, registered, phone, cell, id, picture, nat}= personInfo
      let {title, first, last} = name
      let {street, city, state, country, postcode, coordinates, timezone} = location
      let {number, streetName = personInfo.location.street.name} = street
      let {latitude, longitude} = coordinates
      let {offset, description}= timezone
      let{uuid, username, password, salt, md5, sha1, sha256}= login
      let {date, age} = dob
      let {registrationDate = personInfo.registered.date, registrationAge = personInfo.registered.age} = registered
      let {idName = personInfo.id.name, value} = id
